...
| 5 | Test implementation | テスト実装 | `/qa-kit:ready` | ✅ |
...
2. **Test implementation** (phase 5) — `/qa-kit:ready` đã có (checklist đối chiếu System
   Profile + test case trước khi chạy thật). Vẫn thiếu: tự động hoá chuẩn bị test data + env —
   `/qa-kit:ready` cố ý không làm việc này (QC không tự động hoá hạ tầng), chỉ nhắc tester tự
   soát. Hệ quả: `D_env` vẫn có thể là category triage lớn nếu tester bỏ qua bước
   `/qa-kit:ready`.
